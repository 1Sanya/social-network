...
import {
  CallACT, FollowToggleACT, FriendsACT, FriendsAT, OpenDialogACT,
} from '../../Types/FriendsT';
...
export const followAC = (userId: number): FollowToggleACT => ({ type: FriendsAT.FOLLOW_TOGGLE, userId });

export const OpenDialogAC = (userId: number): OpenDialogACT => ({ type: FriendsAT.OPED_DIALOG, userId });

export const CallAC = (userId: number): CallACT => ({ type: FriendsAT.CALL, userId });
